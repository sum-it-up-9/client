...
import { AddToLikes, RemoveFromLikes } from "../utils/OrderSlice";

const OrdersResCard = (props) => {
    ...
    const handleHeartIconClick = async(e) => {
        ...
        e.stopPropagation();
        if( liked ){
            dispatch(RemoveFromLikes(resData?.data?.id));
        }
        setLiked(!liked);
    };

    const heartIconClass = "ri-heart-fill clr";
